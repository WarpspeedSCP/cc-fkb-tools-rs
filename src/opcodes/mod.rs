    ...
    for i in composition {
        match i {
            OpFieldType::s => {
                ...
                fields.push(OpField::String(string));
                ptr += bytes.len();
            }
            OpFieldType::b => {
            ...
            OpFieldType::c => {
                fields.push(OpField::Choice { n_choices: 0, choices: vec![] });
                ptr += 1;
            }
            _ => panic!()
        }
...
impl OpField {
    fn size(&self) -> usize {
        match self {
            ...
            OpField::Word(_) => 2,
            OpField::DWord(_) => 4,
            OpField::String(inner) => encode_sjis(inner.as_str()).len() + 1,
            OpField::Choice { choices, .. } => choices.iter().map(|choice| choice.size()).sum(),
...
impl Opcode {
    pub(crate) fn size(&self) -> usize {
        1 + self.fields.iter().map(|it| it.size()).sum::<usize>()
    }
}
...
pub fn make_opcode(input: &[u8], addr: usize) -> Option<Opcode> {
    ...
    let res = match &input[0] {
        ...
        0x09 => opcode!(w, s), // make_string_opcode(1, input), //: Variable (minimum 2 bytes + string length)
        ...
        0x0D => opcode!(w, w, d, p), // n_byte_opcode(input, 10), //: 10 bytes (1 + 2 + 2 + 4 + 1)
        ...
        0x23 | 0x27 => opcode!(b, w, w, w, b, b, p), // make_string_opcode(10, input), // 0x27: Variable (minimum 11 bytes + string length)
        ...
        0x48 => opcode!(b, w, w, d, b, b, p), // n_byte_opcode(input, 13), //: 13 bytes (1 + 11 + 1)
